...
class LocalDatabase extends _$LocalDatabase {
  ...
  Future<void> insertReport(CachedReport report) =>
      into(cachedReports).insertOnConflictUpdate(report);
  Future<void> clearAllReports() => delete(cachedReports).go();

  // EmergencyContacts CRUD Helpers
  Future<List<EmergencyContact>> getAllContacts() =>
      select(emergencyContacts).get();
  Stream<List<EmergencyContact>> watchAllContacts() =>
      select(emergencyContacts).watch();
  Future<void> insertContact(EmergencyContact contact) =>
      into(emergencyContacts).insertOnConflictUpdate(contact);
  Future<void> deleteContact(String contactId) => (delete(
    emergencyContacts,
  )..where((tbl) => tbl.id.equals(contactId))).go();

  // BegalHeatmaps CRUD Helpers
  Future<List<BegalHeatmap>> getAllHeatmaps() => select(begalHeatmaps).get();
  Stream<List<BegalHeatmap>> watchAllHeatmaps() =>
      select(begalHeatmaps).watch();
  Future<void> insertHeatmap(BegalHeatmap point) =>
      into(begalHeatmaps).insertOnConflictUpdate(point);
