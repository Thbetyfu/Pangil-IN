  ...
  Widget build(BuildContext context) {
    return BlocBuilder<DashboardBloc, DashboardState>(
      builder: (context, state) {
        ...
        final activeSosCount = state.reports.where((r) => r['status'] == 'PENDING' || r['status'] == 'ON_PROCESS').length;
        final onDutyPatrols = state.patrolUnits.where((u) => u['status'] == 'ON_DUTY').length;
        final availablePatrols = state.patrolUnits.where((u) => u['status'] == 'AVAILABLE').length;
        ...
        if (state.reports.isNotEmpty) {
          mapCenter = LatLng(state.reports.first['latitude'], state.reports.first['longitude']);
        }
                      ...
                      if (isPending)
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            color: markerColor.withOpacity(1.0 - _pulseController.value),
                          ),
                        ),
                      GestureDetector(
                        onTap: () {
                          context.read<DashboardBloc>().add(SelectReportEvent(report));
                          context.read<DashboardBloc>().add(ChangeTabEvent(1)); // Navigate to detail
                        },
                        child: Container(
                          width: 24,
                          height: 24,
                          decoration: BoxDecoration(
                            ...
                            boxShadow: [
                              BoxShadow(color: markerColor.withOpacity(0.5), blurRadius: 10, spreadRadius: 2),
                            ],
                          ),
                          child: Icon(
        ...
        for (var unit in state.patrolUnits) {
          ...
          final Color markerColor = isOnDuty ? SigapTheme.primaryColor : SigapTheme.successColor;
            ...
            Marker(
              ...
              child: Container(
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: Colors.white, width: 2),
                  boxShadow: [
                    BoxShadow(color: markerColor.withOpacity(0.4), blurRadius: 8, spreadRadius: 1),
                  ],
                ),
                child: const Icon(
        ...
        for (var camera in state.cctvCameras) {
          ...
          final Color markerColor = isHighFps ? SigapTheme.primaryColor : Colors.amber;
          ...
          mapMarkers.add(
            Marker(
              ...
              child: GestureDetector(
                onTap: () {
                  context.read<DashboardBloc>().add(ChangeTabEvent(2)); // Live CCTV Monitor
                },
                child: Container(
                  decoration: BoxDecoration(
                ...
                children: [
                  ...
                  Expanded(
                    child: _buildStatCard(
                      ...
                      value: '$onDutyPatrols / ${onDutyPatrols + availablePatrols}',
                        ...
                        children: [
                          ...
                          Expanded(
                            child: state.cctvAlerts.isEmpty
                                ? GlassCard(
                                    ...
                                    child: Center(
                                      child: Column(
                                        ...
                                        children: [
                                          Icon(Icons.shield_rounded, color: Colors.white.withOpacity(0.15), size: 40),
                                          const SizedBox(height: 12),
                                          Text(
                                ...
                                : ListView.builder(
                                    itemCount: state.cctvAlerts.length > 5 ? 5 : state.cctvAlerts.length,
                                    itemBuilder: (context, idx) {
                                      ...
                                      return Padding(
                                        ...
                                        child: GlassCard(
                                          ...
                                          opacity: 0.06,
                                          border: Border.all(color: SigapTheme.primaryColor.withOpacity(0.2)),
                                          child: Row(
                                            children: [
                                              ClipRRect(
                                                ...
                                                child: Image.network(
                                                  ...
                                                  errorBuilder: (context, _, __) => Container(
                                                    ...
                                                    width: 50,
                                                    height: 50,
                                                    child: const Icon(Icons.videocam_off, size: 18),
                                                  ),
                                                ),
                                              ),
                                              ...
                                              Expanded(
                                                child: Column(
                                                  ...
                                                  children: [
                                                    Text(
                                                      'Begal Senjata Tajam',
                                                      style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: SigapTheme.primaryColor),
                                                    ),
                          ...
                          Text(
                            'SIMULASI DETEKSI AI',
                            style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1.5, color: Colors.white.withOpacity(0.4)),
                          ),
                          const SizedBox(height: 8),
                          ElevatedButton.icon(
                            style: ElevatedButton.styleFrom(
                              backgroundColor: SigapTheme.primaryColor.withOpacity(0.15),
                              foregroundColor: SigapTheme.primaryColor,
                              side: const BorderSide(color: SigapTheme.primaryColor),
                            ...
                            onPressed: state.cctvCameras.isEmpty 
                                ...
                                : () {
                                    ...
                                    context.read<DashboardBloc>().add(
                                      TriggerMockSosEvent(
                                        cctvId: camera['id'],
                                        suspectFeatures: 'jaket_hitam_helm_merah_sajam_katana',
                                      ),
                                    );
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                        content: Text('Simulasi alert anomali CCTV berhasil dikirim ke server AI'),
                                        backgroundColor: SigapTheme.primaryColor,
                                      ),
                                    );
                                  },
                            icon: const Icon(Icons.warning_rounded, size: 16),
                            label: const Text('PICU BEGAL SAJAM (MOCK AI)', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11)),
                          )
                        ],
                      ),
                    ),
      ...
      child: Row(
        children: [
          ...
          AnimatedBuilder(
            animation: _pulseController,
            builder: (context, child) {
              final double scale = pulse ? 1.0 + (_pulseController.value * 0.15) : 1.0;
