...
void main() {
  ...
  test('LoginEvent success for police operator', () {
    dashboardBloc.add(LoginEvent(email: 'admin@sigap.go.id', password: 'password123', otpCode: '123456'));
  ...
  test('LoginEvent failure for citizen (unauthorized role)', () {
    dashboardBloc.add(LoginEvent(email: 'citizen@gmail.com', password: 'password123', otpCode: '123456'));
    ...
    expect(
      ...
      emitsThrough(
        predicate<DashboardState>((state) =>
            ...
            state.authError == 'Akses ditolak: Hanya untuk petugas kepolisian'),
      ),
    );
  });
  ...
  test('FetchSuspectAnalysisEvent updates state with AI predictedRoutes and reidPredictions (F-08)', () {
    dashboardBloc.add(FetchSuspectAnalysisEvent(
  ...
  test('GpsUpdateReceivedEvent appends isBleRelay metadata to gpsTrackLogs (F-03)', () {
    // Populate cache with reportId
    dashboardBloc.emit(dashboardBloc.state.copyWith(
      gpsTrackLogs: {'report-uuid-1': []},
    ));

    dashboardBloc.add(GpsUpdateReceivedEvent({
