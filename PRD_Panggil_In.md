...
* **Aturan Bisnis (Business Rules):**
  ...
  3. **Mode Berkendara (Riding Mode):** Aplikasi memantau stream GPS secara berkelanjutan di latar belakang. Jika kecepatan terdeteksi > 15 km/jam (4.167 m/s), status Riding Mode otomatis diaktifkan dan pemicu guncangan (*High-G Gesture*) dinonaktifkan. Sebaliknya, jika kecepatan berada di bawah batas tersebut, Riding Mode dinonaktifkan secara otomatis untuk mencegah *false trigger* akibat guncangan jalan.
...
* **Aturan Bisnis (Business Rules):**
  ...
  2. **Metode 1 (Pelacakan Internet Terhubung):** Mengaktifkan *Persistent Foreground Service* dengan prioritas tertinggi OS. Koordinat GPS dienkripsi secara lokal menggunakan algoritma AES-256-CBC (kunci enkripsi statis 32-byte `panggilin_super_secret_32_bytes_` dan IV 16-byte nol) sebelum dikirimkan ke SIGAP Desktop pusat komando secara *real-time* setiap 2 detik menggunakan protokol komunikasi MQTT dengan tingkat keandalan QoS 1 (At least once).
  3. **Metode 2 - Taktik A (Stealth SMS Ping - Tanpa Paket Data):** Jika *Network State Listener* mendeteksi koneksi internet terputus (dimatikan oleh pelaku), aplikasi otomatis beralih menggunakan jaringan seluler seluler biasa (2G/3G/4G) untuk mengirimkan SMS Latar Belakang (*Stealth SMS*) berisi koordinat GPS terenkripsi menggunakan AES-256-CBC ke SMS Gateway pusat komando setiap 30 detik secara senyap tanpa riwayat SMS.
...
* **Aturan Bisnis (Business Rules):**
  ...
  2. Modul akan menganalisis integritas gambar: pemeriksaan *metadata* EXIF (pemindaian struktur biner JPEG secara lokal untuk mencari penanda APP1 [FFE1] dan header `Exif\0\0`), deteksi duplikasi gambar internet (reverse search / hashing), serta deteksi keaslian piksel gambar untuk memastikan foto diambil langsung dari kamera aplikasi (bukan hasil unduhan internet atau hasil rekayasa AI generatif).
  3. Modul suara SOS juga dianalisis untuk mendeteksi *Deepfake Audio* / suara sintesis AI generatif menggunakan analisis akustik biner WAV untuk mendeteksi rasio keheningan digital mutlak (silence ratio > 40%) dan pemotongan sinyal ekstrem (clipping/saturation artifacts).
